...
const PainPoints = () => {
  ...
  return (
    <section ref={ref} id="pain" className="py-16 md:py-28 relative section-glow-top">
      <div className="container mx-auto px-4 md:px-6">
        <motion.h2
          className="text-2xl sm:text-3xl md:text-5xl font-display font-extrabold text-center mb-10 md:mb-16 leading-tight"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 max-w-4xl mx-auto">
          {painItems.map((item, i) => (
            <motion.div
              key={i}
              className="glass-card p-6 md:p-10 cursor-default group border border-transparent hover:border-primary/20 transition-all duration-500"
              initial={{ opacity: 0, y: 30 }}
              animate={isVisible ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: 0.15 + i * 0.12 }}
              whileHover={{ y: -8, scale: 1.02, transition: { duration: 0.3 } }}
            >
              <div className="text-primary mb-4 md:mb-5 group-hover:text-accent transition-colors duration-300">{item.icon}</div>
              <h3 className="text-lg md:text-xl font-display font-bold text-heading mb-3 md:mb-4 group-hover:text-primary transition-colors duration-300">
                {item.title}
              </h3>
