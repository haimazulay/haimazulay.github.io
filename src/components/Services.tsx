...
const Services = () => {
  ...
  return (
    <section ref={ref} id="services" className="py-16 md:py-32 relative section-glow-top">
      <div className="container mx-auto px-4 md:px-6">
        <motion.h2
          className="text-2xl sm:text-3xl md:text-5xl font-display font-extrabold text-center mb-12 md:mb-20"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
          {services.map((service, i) => (
            <motion.div
              key={i}
              className="glass-card p-6 md:p-10 cursor-default group border border-transparent hover:border-primary/20 transition-all duration-500 relative overflow-hidden"
            ...
            >
              <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                ...
                <h3 className="text-lg md:text-xl font-display font-bold text-heading group-hover:text-primary transition-colors duration-300">
                  {service.title}
                </h3>
              </div>
              <ul className="space-y-2 md:space-y-3">
                {service.items.map((item, j) => (
                  <li key={j} className="flex items-start gap-2 md:gap-3 text-sm md:text-base text-muted-foreground">
                    <svg
                      className="w-4 h-4 text-accent mt-0.5 md:mt-1 shrink-0"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2.5"
